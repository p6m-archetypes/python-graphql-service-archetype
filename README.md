...
An [Archetect](https://archetect.github.io/) (v3 / Lua) archetype that generates a Python GraphQL
service using Strawberry and FastAPI, with a management sidecar (health probes + Prometheus
metrics), CI workflows, and platform (Kubernetes) manifests.
...
- **live** — boots the rendered service and exercises the GraphQL query + mutation over
  `/graphql` and the management sidecar (health probes + Prometheus metrics) over the wire
...
prova tests/graphql_service_test.lua    # a single test file
```
